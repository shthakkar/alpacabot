...
# ---------------- logging ----------------
log = logging.getLogger("bot_1min_nc")
...
_ef = logging.FileHandler(f"logs/events_1min_nc_{_logdate}.log")
...
STALE_DATA_SEC    = 120       # skip cycle if latest bar > 2 min old (covers 1-min bar + IEX latency)
FROZEN_BAR_LIMIT  = 12        # same bar seen 12×10s = 2 min → likely frozen feed
...
class Bot:
    def __init__(self):
        ...
        self.daily_pnl = 0.0
        self._loss_cooldown_until: datetime | None = None  # unused — no cooldown

    # ---------- startup ----------
    def startup_checks(self):
        log.info("=" * 60)
        log.info(" SPY Mean-Reversion Options Bot [1-MIN NO-COOLDOWN]")
        log.info("=" * 60)
        log.info(f"  Mode          : {'PAPER' if PAPER else 'LIVE'}")
        log.info(f"  Poll          : every 10s")
    ...
    def _enter_trade(self, direction: str, spy_price: float, now: datetime):
        ...
        # Step 2: always enter 1 contract, always allow both avg-ups
        entry_qty = 1
        allowed_avg_ups = 2
